...
import net.pinero.simpledeserteagle.procedures.DesertEagleReloadProcedure;
...
public class ReloadMessage {
	...
	public static void pressAction(Player entity, int type, int pressedms) {
		...
		if (!world.hasChunkAt(entity.blockPosition()))
			return;
		if (type == 0) {
			DesertEagleReloadProcedure.execute(world, entity);//换弹关键
		}
	}

	@SubscribeEvent
	public static void registerMessage(FMLCommonSetupEvent event) {
		SimpledeserteagleMod.addNetworkMessage(ReloadMessage.class, ReloadMessage::buffer, ReloadMessage::new, ReloadMessage::handler);
	}
}
